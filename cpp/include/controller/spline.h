...
protected:
    ...
    size_t find_closestY(double x) const;    // closest idx so that m_x[idx]<=x
public:
    ...
    double derivY(int order, double y) const;
                        ...
                        spline_type type)
{
    ...
    m_y=y;
    int n = (int) x.size();
    // check strict monotonicity of input vector x   (comment by lq) 
    // for(int i=0; i<n-1; i++) {
...
size_t spline::find_closestY(double x) const
{
    std::vector<double>::const_iterator it;
    it=std::upper_bound(m_y.begin(),m_y.end(),x);       // *it > x
    size_t idx = std::max( int(it-m_y.begin())-1, 0);   // m_x[idx] <= x
    return idx;
}
...
double spline::derivY(int order, double x) const
{
    assert(order>0);
    size_t n=m_y.size();
    size_t idx = find_closestY(x);
